...
pipeline {
    ...
    stages {
        ...
        stage('Test Backend') {
            ...
            steps {
                sh '''# aggressive cleanup of any previous test-db containers
                    docker rm -f test-db-${BUILD_NUMBER} test-db 2>/dev/null || true
            ...
            post {
                always {
                    sh 'docker rm -f test-db-${BUILD_NUMBER} test-db 2>/dev/null || true'
